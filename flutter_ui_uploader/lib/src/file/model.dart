...
class FileUploadControllerModel<T> with ChangeNotifier {
  ...
  Future<void> _upload({required bool retry}) async {
    try {
      ...
      // Xử lý đặc biệt cho FileAlreadyUploadedException
      if (e is FileAlreadyUploadedException) {
        _status = FileUploadStatus.done;
        _result ??= FileUploadResult(
          file: _ref.file,
          id: generateUniqueId(),
        );
      } else {
